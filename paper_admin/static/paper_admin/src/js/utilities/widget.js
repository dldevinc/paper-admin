...
class Widget {
     ...
     * Регистрация обработчика позволяет удалить его с помощью
     * "removeListener()" или подобной функции.
     *
     * Включение селектора в имя события позволяет разграничить обработчики,
     * добавляемые серией вызовов метода "observe()" для разных селекторов.
     * Если же обработчик глобальный, то его можно зарегистрировать в методе
     * "constructor", не указывая селектор.
     *
     * @param {String} name
